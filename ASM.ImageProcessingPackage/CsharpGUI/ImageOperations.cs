...
namespace CsharpGUI
{
    ...
    public class ImageOperations
    {
        ...
        public static byte[] convert_to_1D(RGBPixel[,] img_matrix, int width, int hight)
        {
            byte[] arr = new byte[3*width * hight];
            ...
            for (int i = 0; i < hight; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    arr[count] = img_matrix[i, j].red;
                    count++;
                    arr[count] = img_matrix[i, j].green;
                    count++;
                    arr[count] = img_matrix[i, j].blue;
                    count++;
                }
            }
            return arr;
        }
        public static void convert_to_RGB_1D(RGBPixel[,] img_matrix, int width, int hight , int Window_Width , byte[] red_arr , byte[] green_arr , byte[] blue_arr)
        {
            ...
            /*
             red_arr = new byte[size];
             green_arr = new byte[size];
            blue_arr = new byte[size];
            */
            ...
            for (int i = 0; i < hight ; i++)
            {
                for (int j = 0; j < width ; j++)
                {    
                        red_arr[count] = img_matrix[i, j].red;
                        green_arr[count] = img_matrix[i, j].green;
                        blue_arr[count] = img_matrix[i, j].blue;
                        count++;
